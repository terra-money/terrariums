...
export class Deployer {
  ...
  public buildContract(contract: string, spinner?: Ora) {
    ...
    spinner?.start();
    execSync('cargo wasm', { stdio: spinner ? 'ignore' : 'inherit' });
    spinner?.succeed();
    process.chdir(cwd);
  }

  public optimizeContract(contract: string, spinner?: Ora) {
    if (!this.config.contracts[contract]) {
      throw new Error(
        `Contract ${contract} build information not found in config file.`,
      );
    ...
    if (pkg.metadata?.scripts?.optimize) {
      ...
      spinner?.start();
      execSync(optimizeCmd, { stdio: spinner ? 'ignore' : 'inherit' });
    } else {
