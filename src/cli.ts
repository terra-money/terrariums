...
if (argv._[0] === "deploy") {
  ...
  if (buildInfo.deploy_script) {
    ...
    const child = fork(
      path.join(process.cwd(), buildInfo.deploy_script),
      args,
      {
        env: process.env,
    ...
    task(async ({ deployer }) => {
      deployer.buildContract(argv.contract);
      deployer.optimizeContract(argv.contract);

      await deployer.storeCode(argv.contract);
      await new Promise((resolve) => setTimeout(resolve, 3000));
      await deployer.instantiate(argv.contract, buildInfo.instantiate_msg);
    }).then(() => {
      process.exit(0);
    });
  } else {
